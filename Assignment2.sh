...
ADD 'dennis' TO sudo IF NOT ALREADY
CREATE ~/.ssh AND SET CORRECT PERMISSIONS 
GENERATE RSA + ED25519 KEYS IF MISSING
ADD PUBLIC KEYS TO authorized_keys WITHOUT DUPLICATES
ADD EXTRA PUBLIC KEY FOR 'dennis'
LOG ALL ACTIONS AND MAKE PROCESS SAFE TO RERUN
COMMENT
